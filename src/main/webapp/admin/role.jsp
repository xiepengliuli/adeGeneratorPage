...
<c:if test="${fn:contains(LOGIN_USER.moduleList, 'admin/role/editPage')}">
  <script type="text/javascript">
      $.canEdit = true;
    </script>
</c:if>
<c:if test="${fn:contains(LOGIN_USER.moduleList, 'admin/role/delete')}">
  <script type="text/javascript">
      $.canDelete = true;
    </script>
</c:if>
<c:if test="${fn:contains(LOGIN_USER.moduleList, 'admin/role/grantPage')}">
  <script type="text/javascript">
      $.canGrant = true;
    </script>
</c:if>
<script type="text/javascript">
  ...
  $(function() {
    treeGrid = $('#treeGrid')
        .treegrid(
            {
              url : '${pageContext.request.contextPath}/admin/role/treeGrid',
              idField : 'id',
              treeField : 'roleName',
  ...
  function editFun(id) {
    ...
    if (node) {
      parent.$.modalDialog({
        ...
        width : 500,
        height : 300,
        href : '${pageContext.request.contextPath}/admin/role/editPage?id=' + node.id,
        buttons : [ {
          text : '编辑',
          handler : function() {
  ...
  function addFun() {
    parent.$.modalDialog({
      ...
      width : 500,
      height : 300,
      href : '${pageContext.request.contextPath}/admin/role/addPage',
      buttons : [ {
        text : '添加',
        handler : function() {
          parent.$.modalDialog.openner_treeGrid = treeGrid;//因为添加成功之后，需要刷新这个treeGrid，所以先预定义好
  ...
  function grantFun(id) {
    ...
    if (node) {
      parent.$.modalDialog({
        ...
        width : 500,
        height : 500,
        href : '${pageContext.request.contextPath}/admin/role/grantPage?id=' + node.id,
        buttons : [ {
          text : '授权',
          handler : function() {
            parent.$.modalDialog.openner_treeGrid = treeGrid;//因为添加成功之后，需要刷新这个treeGrid，所以先预定义好
...
<body>
  ...
  <div id="toolbar" style="display: none;">
    <c:if test="${fn:contains(LOGIN_USER.moduleList, 'admin/role/addPage')}">
  ...
  <div id="menu" class="easyui-menu" style="width: 120px; display: none;">
    <c:if test="${fn:contains(LOGIN_USER.moduleList, '/roleController/addPage')}">
      <div onclick="addFun();" data-options="iconCls:'pencil_add'">增加</div>
    </c:if>
    <c:if test="${fn:contains(LOGIN_USER.moduleList, '/roleController/delete')}">
      <div onclick="deleteFun();" data-options="iconCls:'pencil_delete'">删除</div>
    </c:if>
    <c:if test="${fn:contains(LOGIN_USER.moduleList, '/roleController/editPage')}">
