  ...
  $(function() {
    ...
    $('#form').form({
      ...
      success : function(result) {
        ...
        if (result.success) {
          parent.$.messager.alert('提示', '添加权限成功，刷新界面或者重新登录后生效！', 'info');
          ...
          <td><select name="moduleTypeId" class="easyui-combobox"
            data-options="width:140,height:29,editable:false,panelHeight:'auto',readonly:true">
              <c:forEach items="${moduleTypeList}" var="moduleType">
                <option value="${moduleType.id}" <c:if test="${moduleType.id =='0'}">selected</c:if>>${moduleType.name}</option>
              </c:forEach>
          </select></td>
        </tr>
        <tr>
          <th>排序</th>
          <td><input name="moduleSort" value="100" class="easyui-numberspinner"
