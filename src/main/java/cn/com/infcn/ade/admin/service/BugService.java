...
import cn.com.infcn.core.model.Tbug;
...
public class BugService {
    ...
    @Autowired
    private BaseDaoI<Tbugtype> bugTypeDao;
    ...
    public EasyUIDataGrid dataGrid(Bug bug, PageHelper ph) {
        ...
        List<Tbug> l = bugDao.find(hql + joinHql + whereHql(bug, params) + orderHql(ph), params, ph.getPage(),
                ph.getRows());
        if (l != null && l.size() > 0) {
            for (Tbug t : l) {
                Bug b = new Bug();
        ...
        dg.setRows(bl);
        dg.setTotal(
                bugDao.count("select count(*) " + hql + " left join t.tbugtype type " + whereHql(bug, params), params));
        return dg;
    }
